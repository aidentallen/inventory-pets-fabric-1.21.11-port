...
public class petMishumaaSaba
extends Item {
    private boolean giftChk = false;

    public petMishumaaSaba(Item.Properties properties) {
        super(properties);
    }
    ...
    public void inventoryTick(ItemStack stack, Level worldIn, Entity entityIn, int slot, boolean par5) {
        ...
        if (KeyHandler.nflag && worldIn.isClientSide && petchk != ItemStack.EMPTY && petchk.getItem() == InventoryPets.PET_MISHUMAA_SABA.get()) {
    ...
    public InteractionResultHolder<ItemStack> use(Level worldIn, Player playerIn, InteractionHand handIn) {
        if (((Boolean)InventoryPetsConfig.disableMishumaaSaba.get()).booleanValue()) {
