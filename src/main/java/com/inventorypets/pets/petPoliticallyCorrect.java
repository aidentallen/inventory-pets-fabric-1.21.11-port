...
public class petPoliticallyCorrect
extends Item {
    private boolean giftChk = false;

    public petPoliticallyCorrect(Item.Properties properties) {
        super(properties);
    }
    ...
    public void inventoryTick(ItemStack stack, Level worldIn, Entity entityIn, int slot, boolean par5) {
        ...
        if (KeyHandler.nflag && worldIn.isClientSide && petchk != ItemStack.EMPTY && petchk.getItem() == InventoryPets.PET_POLITICALLY_CORRECT.get()) {
            this.petNamer(entityplayer);
            KeyHandler.nflag = false;
        } else if (KeyHandler.nflag && petchk == ItemStack.EMPTY) {
            KeyHandler.nflag = false;
        }
        if (((Boolean)InventoryPetsConfig.disablePoliticallyCorrect.get()).booleanValue()) {
            return;
        }
    }
        ...
        if (!worldObj.isClientSide) {
            ...
            if (mMonth == (Integer)InventoryPetsConfig.holidayPetMonth.get() - 1 && mDay >= 1 && mDay <= 25) {
                ...
                if (this.giftChk) {
                    ...
                    ItemEntity entityitem = new ItemEntity(entityplayer.level(), entityplayer.getX() + 0.5, entityplayer.getY() + 0.5, entityplayer.getZ() + 0.5, itemstack);
                    worldObj.addFreshEntity((Entity)entityitem);
                    worldObj.playSound(null, entityplayer.getX(), entityplayer.getY(), entityplayer.getZ(), ModSoundEvents.sleigh.get(), SoundSource.PLAYERS, 0.6f, 1.0f);
                } else if (!this.giftChk && mDay >= 14 && mDay <= 25) {
                    entityplayer.sendSystemMessage((Component)Component.translatable((String)"item.gift.taken", (Object[])new Object[0]));
                } else {
                    entityplayer.sendSystemMessage((Component)Component.translatable((String)"pet.holiday.dates", (Object[])new Object[0]));
                }
            } else {
                worldObj.playSound(null, entityplayer.getX(), entityplayer.getY(), entityplayer.getZ(), ModSoundEvents.raspberry.get(), SoundSource.PLAYERS, 0.3f, 0.7f);
                entityplayer.sendSystemMessage((Component)Component.translatable((String)"item.gift.notholiday", (Object[])new Object[0]));
            }
        }
