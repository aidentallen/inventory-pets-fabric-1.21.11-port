...
extends Item {
    public static final int[] META_SCORES = new int[]{0, 10, 20, 30, 40, 50, 60, 70, 100, 101, 102, 103};
    RandomPoolAlias rand = new RandomPoolAlias();
    private int basePet = 0;

    public petMeta(Item.Properties properties) {
        super(properties);
    }
    ...
    public void removeItem(Player ep, ItemStack removeitem) {
        ...
        if (inventoryPlayer.getItem(i = ep.getInventory().selected) != ItemStack.EMPTY && (j = inventoryPlayer.getItem(i)) != ItemStack.EMPTY && j.getItem() == removeitem.getItem()) {
