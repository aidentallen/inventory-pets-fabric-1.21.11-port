...
public class SteveBallmerEntity
extends WanderingTrader {
    public SteveBallmerEntity(EntityType<? extends SteveBallmerEntity> entityBillGates, Level worldIn) {
        super(entityBillGates, worldIn);
        this.getNavigation().setCanFloat(true);
        this.setCanPickUpLoot(true);
        this.setCustomName((Component)Component.literal((String)"Steve Ballmer"));
        this.setAggressive(true);
        this.setSprinting(true);
    }

    protected void updateTrades() {
        CEOTrades.ItemListing[] avillagertrades$itrade = (CEOTrades.ItemListing[])CEOTrades.STEVE_BALLMER.get(1);
        CEOTrades.ItemListing[] avillagertrades$itrade1 = (CEOTrades.ItemListing[])CEOTrades.STEVE_BALLMER.get(2);
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return Animal.createMobAttributes().add(Attributes.MAX_HEALTH, 20.0).add(Attributes.MOVEMENT_SPEED, (double)0.8f).add(Attributes.FOLLOW_RANGE, 48.0).add(Attributes.ATTACK_DAMAGE, 0.0);
    }

    protected SoundEvent getAmbientSound() {
        return this.isTrading() ? SoundEvents.WANDERING_TRADER_TRADE : ModSoundEvents.sbsay.get();
    }
